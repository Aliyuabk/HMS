...
$sql = mysqli_query($con, "SELECT * FROM lab WHERE id='$user_id'");
...
if (isset($_POST['change_password'])) {
    ...
    if (password_verify($current_password, $user_data['password'])) {
        if ($new_password === $confirm_password) {
            ...
            mysqli_query($con, "UPDATE lab SET password='$hashed' WHERE id='$user_id'");
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            if (!is_dir('uploads/labs')) {
                mkdir('uploads/labs', 0777, true);
            }

            $new_name = "lab_" . $user_id . "." . $ext;
            $path = "uploads/labs/" . $new_name;

            if (move_uploaded_file($_FILES['photo']['tmp_name'], $path)) {
                mysqli_query($con, "UPDATE lab SET photo='$path' WHERE id='$user_id'");
...
<html lang="en">
<head>
    <title>lab | Profile Settings</title>
...
<div class="row">
<div class="col-sm-8">
<h2 class="mainTitle">lab | Profile Settings</h2>
</div>
</div>
</section>
