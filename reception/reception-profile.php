...
if (isset($_POST['change_password'])) {
    ...
    if (password_verify($current_password, $user_data['password'])) {
        if ($new_password === $confirm_password) {
            ...
            mysqli_query($con, "UPDATE reception SET password='$hashed' WHERE id='$user_id'");
...
if (isset($_POST['upload_photo'])) {
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            if (!is_dir('uploads/reception')) {
                mkdir('uploads/reception', 0777, true);
            }

            $new_name = "reception_" . $user_id . "." . $ext;
            $path = "uploads/reception/" . $new_name;

            if (move_uploaded_file($_FILES['photo']['tmp_name'], $path)) {
                mysqli_query($con, "UPDATE reception SET photo='$path' WHERE id='$user_id'");
                echo "<script>alert('Profile photo updated'); window.location='reception-profile.php';</script>";
            } else {
...
<div class="row">
<div class="col-sm-8">
<h2 class="mainTitle">reception | Profile Settings</h2>
</div>
</div>
</section>
