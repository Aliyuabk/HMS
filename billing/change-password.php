...
$sql = mysqli_query($con, "SELECT * FROM billing WHERE id='$user_id'");
...
if (isset($_POST['change_password'])) {
    ...
    if (password_verify($current_password, $user_data['password'])) {
        if ($new_password === $confirm_password) {
            ...
            mysqli_query($con, "UPDATE billing SET password='$hashed' WHERE id='$user_id'");
...
if (isset($_POST['upload_photo'])) {
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            if (!is_dir('uploads/billing')) {
                mkdir('uploads/billing', 0777, true);
            }

            $new_name = "billing_" . $user_id . "." . $ext;
            $path = "uploads/billing/" . $new_name;

            if (move_uploaded_file($_FILES['photo']['tmp_name'], $path)) {
                mysqli_query($con, "UPDATE billing SET photo='$path' WHERE id='$user_id'");
