...
if (!isset($_SESSION['user_id']) || $_SESSION['role'] !== 'radiology') {
    header("Location: ../index.php");
    exit;
}

$user_id = $_SESSION['user_id'];
$sql = mysqli_query($con, "SELECT * FROM radiology WHERE id='$user_id'");
...
if (isset($_POST['change_password'])) {
    ...
    if (password_verify($current_password, $user_data['password'])) {
        if ($new_password === $confirm_password) {
            ...
            mysqli_query($con, "UPDATE radiology SET password='$hashed' WHERE id='$user_id'");
...
if (isset($_POST['upload_photo'])) {
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            if (!is_dir('uploads/radiology')) {
                mkdir('uploads/radiology', 0777, true);
            }

            $new_name = "radiology_" . $user_id . "." . $ext;
            $path = "uploads/radiology/" . $new_name;

            if (move_uploaded_file($_FILES['photo']['tmp_name'], $path)) {
                mysqli_query($con, "UPDATE radiology SET photo='$path' WHERE id='$user_id'");
...
<html lang="en">
<head>
    <title>radiology | Profile Settings</title>
...
<div class="row">
<div class="col-sm-8">
<h2 class="mainTitle">radiology | Profile Settings</h2>
</div>
</div>
</section>
...
<form method="post">
<div class="form-group">
<radiologyel>Current Password</radiologyel>
<input type="password" name="current_password" class="form-control" required>
</div>

<div class="form-group">
<radiologyel>New Password</radiologyel>
<input type="password" name="new_password" class="form-control" required>
</div>

<div class="form-group">
<radiologyel>Confirm Password</radiologyel>
