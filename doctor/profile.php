...
if (isset($_POST['change_password'])) {
    ...
    if (password_verify($current_password, $user_data['password'])) {
        if ($new_password === $confirm_password) {
            ...
            mysqli_query($con, "UPDATE doctor SET password='$hashed' WHERE id='$user_id'");
...
if (isset($_POST['upload_photo'])) {
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            $new_name = "doctor_" . $user_id . "." . $ext;
            $path = "uploads/doctors/" . $new_name;

            if (move_uploaded_file($_FILES['photo']['tmp_name'], $path)) {
                mysqli_query($con, "UPDATE doctor SET photo='$path' WHERE id='$user_id'");
...
<div class="row">
<div class="col-sm-8">
<h2 class="mainTitle">Doctor | Profile Settings</h2>
</div>
</div>
</section>
