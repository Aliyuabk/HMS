...
$sql = mysqli_query($con, "SELECT * FROM Admin WHERE id='$user_id'");
...
if (isset($_POST['change_password'])) {
    ...
    if (password_verify($current_password, $user_data['password'])) {
        if ($new_password === $confirm_password) {
            ...
            mysqli_query($con, "UPDATE admin SET password='$hashed' WHERE id='$user_id'");
...
if (isset($_POST['upload_photo'])) {
    ...
    if (!empty($_FILES['photo']['name'])) {
        ...
        if (!in_array($ext, $allowed)) {
            ...
            $new_name = "admin_" . $user_id . "." . $ext;
            $path = "uploads/admin/" . $new_name;

            if (move_uploaded_file($_FILES['photo']['tmp_name'], $path)) {
                mysqli_query($con, "UPDATE admin SET photo='$path' WHERE id='$user_id'");
                echo "<script>alert('Profile photo updated'); window.location='change-password.php';</script>";
            } else {
